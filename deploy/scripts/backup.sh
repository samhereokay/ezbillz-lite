...
docker exec -t $DB_CONTAINER pg_dump -U ezbillz -F c -d ezbillz > "$BACKUP_DIR/db_$TIMESTAMP.dump"
echo "Database backup complete."

# 2. Storage Backup (MinIO)
echo "Backing up storage..."
MINIO_CONTAINER=$(docker compose -f deploy/docker-compose.prod.yml ps -q minio)
...
docker run --rm -v "minio_data_prod:/data:ro" -v "$PWD/$BACKUP_DIR:/backup" alpine sh -c "cp -r /data /backup/storage"
